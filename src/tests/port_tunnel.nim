...
var pstr{.threadvar.}:string ;pstr = newString(len = 2)
var tport:uint16 = 443
copyMem(addr pstr[0],addr tport,2)
...
method read*(self: TestAdapter, bytes: int, chain: Chains = default): Future[StringView] {.async.} =
    ...
    var res:StringView = case index:
            of 0:
                sv.write( pstr & "1234");sv

            of 1:
                sv.write( pstr & "salam farmande");sv
            else:
                nil
    inc index
    return res
...
suite "Suite for testing Port Tunnel":
