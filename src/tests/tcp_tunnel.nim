...
const tcpflag*: char = char(0b00000001)
...
method write*(self: TestAdapter, rp: StringView, chain: Chains = default): Future[void] {.async.} =
    ...
    case index:
        of 0:
            check $sv == (tcpflag & "salam")

        of 1:
            check $sv == (tcpflag & "farmande")
        else:
            discard
...
method read*(self: TestAdapter, bytes: int, chain: Chains = default): Future[StringView] {.async.} =
    ...
    var res: StringView = case index:
        of 0:
            sv.write(tcpflag & "1234"); sv

        of 1:
            sv.write(tcpflag & "salam farmande"); sv
        else:
            nil
    inc index
    return res
...
suite "Suite for testing Tcp Tunnel":
